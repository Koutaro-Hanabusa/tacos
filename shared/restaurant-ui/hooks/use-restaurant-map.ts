...
function createMarkerElement(marker: RestaurantMarker, onSelect: (id: number) => void) {
  const element = document.createElement("button");
  element.type = "button";
  element.className = "restaurant-map-marker";
  element.setAttribute("aria-label", `${marker.name || "名前のないレストラン"}を選択`);
  element.title = marker.name || "名前のないレストラン";
    ...
    markers.forEach((marker) => {
      ...
      if (existing) {
        existing.element.setAttribute(
          "aria-label",
          `${marker.name || "名前のないレストラン"}を選択`,
        );
        existing.element.title = marker.name || "名前のないレストラン";
